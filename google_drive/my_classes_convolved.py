...
from keras.layers import Input, concatenate, Conv2D, MaxPooling2D, Conv2DTranspose, UpSampling2D,\
                         Dense, LeakyReLU, Reshape, Flatten
import sys
import random
sys.path += ['..']
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self):
        ...
        for i, ID in enumerate(self.list_IDs_temp):
              map_tmp = self.data[i, :, :, 0]
              tmp = np.log10(map_tmp * self.conv_const_temp[i])
              tmp = NormalizeData(tmp, 3.3, -3.1)
              X[i, :, :, :] = tmp.reshape((self.dim[0], self.dim[1], self.n_channels))
              self.cv.conv_map(map_tmp)
              conv_map_tmp = self.cv.magcon
              conv_map_tmp = np.log10(conv_map_tmp)
              # conv_map_tmp[conv_map_tmp<=0] = 10**-14.138109839567305
              conv_map_tmp = NormalizeData(conv_map_tmp, 3.3, -3.1)

              Y[i, ] = conv_map_tmp.reshape((self.dim[0], self.dim[1], 1))
        return X, Y
    ...
    #     return X
    def test_maps(self):
        mapID = random.choice(self.indexes)

        map_tmp = self.data[mapID, :, :, 0]
        tmp = np.log10(map_tmp * self.conv_const_temp[mapID])
        tmp = NormalizeData(tmp, 3.3, -3.1)
        X[i, :, :, :] = tmp.reshape((self.dim[0], self.dim[1], self.n_channels))
        self.cv.conv_map(map_tmp)
        conv_map_tmp = self.cv.magcon
