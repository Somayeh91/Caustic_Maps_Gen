...
def process_KS(maps):
	map_1 = maps[0]
	map_2 = maps[1]
	ks_metric = calculate_ks_metric(map_1, map_2)
	return ks_metric

def calculate_ks_metric(map1, map2):
...
def calculate_lc_metric(all_lc, stat_1=np.mean, stat_2=np.mean):
	...
	First dimension is for true maps' lightcurves and AD maps' lightcurves, 
	the second dimension is number of individual maps,
	the third dimension is for number of individual lightcurves per map,
	and the last dimension is the number of data points in the lightcurves.

	The function comapre_lc_metric is calculated for 
	all the lightcurves of all the maps and a single value is returned.
