...
def metric_comparison_plot(metric, metric_name, imgs, plots_dir):
	plt.figure()
	data = models_info['data']
	job_names = models_info['job_names']
	columns = models_info['columns']
	rows = models_info['rows']
...
def metric_comparison_per_rsrc_plot(rsrcs, metric, metric_name, plots_dir):
...
def plot_kgs_lc_distance_metric(kgs,
							    lc_metric, 
							    kgs_ref, 
							    kgs_closest, 
							    output_direc,
	...
	if hasattr(kgs_ref, "__len__"):
		...
		plt.scatter([kgs_closest[plot_x]], [kgs_closest[plot_y]], c='IndianRed', marker='x', s=40, label='Predicted %s'%type_)
		plt.legend()
