...
def lc_gen(map_,
           ...
           dt=(22 * 365) / 1.5,
           # from Kochanek (2004) for Q2237+0305: The quasar takes 22 years to go through 1.5 R_Ein
           set_seed=42):
...
def lc_gen_one(map__, seedd):
    if not np.isnan(seedd):
        random.seed(seedd)
    else:
        random.seed()
    # print('will generate lc for step %i.'%seedd)
    ang = random.randint(0, 360)
    t = np.nan
    while isinstance(t, float):
        if not np.isnan(seedd):
